...
def generate_cam(image_path, ckpt_path, save_dir='./figures/heatmap/'):
    """
    ...
    # show cam
    fusion_rgb = cv2.cvtColor(fusion, cv2.COLOR_BGR2RGB)
    plt.imshow(fusion_rgb)
    plt.axis("off")

    plt.savefig(save_dir + img_name, bbox_inches='tight', pad_inches=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt-path', type=str, required=False, default='./pretrained/cnn/resnet18-sam.pth',
                        help='checkpoints path for inference')
    parser.add_argument('--image-path', type=str, required=False, default='./imgs/normal1.jpeg',
